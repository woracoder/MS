 ...
 *
 */
package com.ir.lbs.client.geonames;
...
public class WebService {
	...
	private static String USER_AGENT = "gnwsc/1.1.10";
	...
	private static Toponym getToponymFromElement(Element toponymElement) {
		...
		Element boundingBoxElement = toponymElement.getChild("bbox");
		BoundingBox bbox = new BoundingBox(0, 0, 0, 0);
		
		if (null != boundingBoxElement) {
			for (Object obj : boundingBoxElement.getChildren()) {
				Element bboxElement = (Element) obj;
				if (bboxElement.getName().equalsIgnoreCase("west")) {
					bbox.setWest(Double.valueOf(bboxElement.getValue()));
				} else if (bboxElement.getName().equalsIgnoreCase("east")) {
					bbox.setEast(Double.valueOf(bboxElement.getValue()));
				} else if (bboxElement.getName().equalsIgnoreCase("north")) {
					bbox.setNorth(Double.valueOf(bboxElement.getValue()));
				} else {
					bbox.setSouth(Double.valueOf(bboxElement.getValue()));
				}
			}
		}

		toponym.setBoundingBox(bbox);
		
		/*for (Object obj : toponymElement.getChildren()) {
			Element element = (Element)obj;
			System.out.println(element.getName() + "/" + element.getValue() + "/" + element.getText());
			for (Object obj1 : element.getChildren()) {
				Element ele = (Element)obj1;
				System.out.println(ele.getName() + "/" + ele.getValue() + "/" + ele.getText());
			}
		}*/
