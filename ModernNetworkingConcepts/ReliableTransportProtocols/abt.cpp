...
//Variable used to assign sequence numbers to the packets being sent from A to B
int seqNoA;

//Variable that denotes if A is waiting for a response from B
bool isWaitingA;

//Variable that assigns the timeout value for
float timeoutA;

//The packet to be sent to B from A through the unreliable channel
struct pkt packetA;

//The acknowledgment number that B should actually send to A when all goes right
int ackNoA;

//Variable used by B to compare sequence numbers of the packets received from A
int seqNoB;
...
void A_output(struct msg message) //ram's comment - students can change the return type of the function from struct to pointers if necessary
{
	...
	//First make the packet to send to B through the unreliable channel
	//Do this only if A is not waiting for any response from B
	if(!isWaitingA) {

		//Copy the message data char array into the packet pay load char array
		memcpy(packetA.payload, message.data, 20);

		//As we need to have alternating sequence numbers for the packets we send so we just flip the values each time
		if(seqNoA == 0) {
			seqNoA = 1;
		} else {
			seqNoA = 0;
		...
		//Start the timer now for the timeout so that we can retransmit after timeout interrupt occurs
		starttimer(0, timeoutA);

		//Set the state to waiting now as the packet is now in transit
		isWaitingA = true;

		//Set the value of the acknowledgment number to what is expected from B so that we can identify error scenario
		ackNoA = seqNoA;
	}
}
...
{
	//Check if the packet delivered is not corrupt and has the expected acknowledgment number
	if(packet.acknum == ackNoA && packet.checksum == calculateChecksum(packet)) {

		cout << "A: Received acknowledgment from B." << endl;

		//Stop the timer for A
		stoptimer(0);

		//Set the waiting state for A to false again
		isWaitingA = false;

	}

	//Else ignore the received packet
	else {
		cout << "Received out of order or corrupted packet. Dropping it." << endl;
	}
}
...
void A_timerinterrupt() //ram's comment - changed the return type to void.
{
	...
	//Set the timeout interval again
	starttimer(0, timeoutA);
}
...
{
	//Set the sequence number for A to 1 initially as it will be inverted every time a new packet is sent to B.
	seqNoA = 1;
	...
	//Set the waiting flag to false for A initially.
	isWaitingA = false;
...
void B_input(struct pkt packet)
{
	...
	if(packet.seqnum == seqNoB && packet.checksum == calculateChecksum(packet)) {
		...
		packetB.acknum = seqNoB;

		//As we need to have alternating sequence numbers for the packets we expect to receive from A
		//so we just flip the values each time for the expected sequence numbers
		if(seqNoB == 0) {
			seqNoB = 1;
		} else {
			seqNoB = 0;
	...
	//Incorrect packet received so set the acknowledgment number opposite to what was the expected sequence number
	else {

		cout << "B: Received out or order or corrupt packet at layer 3 so setting acknowledgment number accordingly." << endl;
	...
	//Send the acknowledgment packet from B to A through the unreliable channel
	tolayer3(1, packetB);

	cout << "B: Sent acknowledgment to layer 3." << endl;
}
...
{
	//Set the expected sequence number to 0 for B as that is what is expected from A initially.
	seqNoB = 0;
...
terminate:
   ...
   cout << endl;
   cout << "Protocol: Alternating-Bit" << endl;
