...
public class DelaysMapper extends
	...
	public void map(Object key, Text value, Context context)
		...
		if(!cols[0].equals("Year")) {
			...
			if(cols[headerMap.get(ARR_DELAY)].equals("NA") || Integer.parseInt(cols[headerMap.get(ARR_DELAY)]) > 0) {
				// Key is (Year Month UniqueCarrier Origin Dest)
				// Val is (ArrDelay CarrierDelay WeatherDelay NASDelay SecurityDelay
				// LateAircraftDelay Cancelled CancellationCode Diverted) and
				// Count CancelledCount (CancelledReason 1-5 individual counts) will be
				// calculated by the reducer
	...
	private String populateKeys() {
		...
		sb.append(cols[headerMap.get(MONTH)] + SPACE);
		sb.append(cols[headerMap.get(UNIQUE_CARRIER)] + SPACE);
		sb.append(cols[headerMap.get(ORIGIN)] + SPACE);
		sb.append(cols[headerMap.get(DEST)] + SPACE);
		return sb.toString();
	}
