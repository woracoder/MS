...
public class CascadingDelaysReducer extends Reducer<Text, Text, Text, Text> {
	...
	public void reduce(Text key, Iterable<Text> values, Context context)
		...
		int cascadeLength = 0, numberOfCascades = 0, totalCascadeTime = 0;
		...
		for (Text val : values) {
			...
			numberOfCascades++;
			if (valCols[positionMap.get(ORIGIN)].equals(valCols[positionMap
					.get(DESTINATION)])) {
				cascadeLength++;
				totalCascadeTime += Integer.parseInt(valCols[positionMap
						.get(ARR_DELAY)]);
			}
		}

		sbv.append((int) cascadeLength / (1.0 * numberOfCascades) + SPACE);
		sbv.append((int) totalCascadeTime / (1.0 * numberOfCascades) + SPACE);
