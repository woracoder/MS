...
int printSortedNumbers(vector<float> buckets[], double time_spent, int size, int argc);
...
int main(int argc, char*argv[]) {
	...
	printSortedNumbers(buckets, time_spent, size, argc);
	return EXIT_SUCCESS;
}
...
int printSortedNumbers(vector<float> buckets[], double time_spent, int size, int argc) {
	FILE *tt = fopen("TimeTakenBucketSortSequential.txt", "a+");
	if (tt == NULL) {
	...
	fprintf(tt, "Total time required in seconds = %f for sorting %d numbers.\n", (time_spent / 1000000.0), size);
	fclose(tt);
	return EXIT_SUCCESS;
}
